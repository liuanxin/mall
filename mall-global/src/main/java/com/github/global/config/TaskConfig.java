...
public class TaskConfig implements AsyncConfigurer {
    ...
    public Executor getAsyncExecutor() {
        ...
        // 如果 cpu 核心是   1, 则最前面的   1 个异步处理, 接下来的    64 个(左移 6 位 = * 64)放进队列, 之后的 2 个(   1 + 2 -   1)异步处理, 再往后的拒绝
        // 如果 cpu 核心是   2, 则最前面的   1 个异步处理, 接下来的   128 个(左移 6 位 = * 64)放进队列, 之后的 3 个(   2 + 2 -   1)异步处理, 再往后的拒绝
        // 如果 cpu 核心是   4, 则最前面的   3 个异步处理, 接下来的   256 个(左移 6 位 = * 64)放进队列, 之后的 3 个(   4 + 2 -   3)异步处理, 再往后的拒绝
        // 如果 cpu 核心是   8, 则最前面的   7 个异步处理, 接下来的   512 个(左移 6 位 = * 64)放进队列, 之后的 3 个(   8 + 2 -   7)异步处理, 再往后的拒绝
        // 如果 cpu 核心是  16, 则最前面的  15 个异步处理, 接下来的  1024 个(左移 6 位 = * 64)放进队列, 之后的 3 个(  16 + 2 -  15)异步处理, 再往后的拒绝
        // 如果 cpu 核心是  32, 则最前面的  31 个异步处理, 接下来的  2048 个(左移 6 位 = * 64)放进队列, 之后的 3 个(  32 + 2 -  31)异步处理, 再往后的拒绝
        // 如果 cpu 核心是  64, 则最前面的  63 个异步处理, 接下来的  4096 个(左移 6 位 = * 64)放进队列, 之后的 3 个(  64 + 2 -  63)异步处理, 再往后的拒绝
        // 如果 cpu 核心是 128, 则最前面的 127 个异步处理, 接下来的  8192 个(左移 6 位 = * 64)放进队列, 之后的 3 个( 128 + 2 - 127)异步处理, 再往后的拒绝
        // 如果 cpu 核心是 256, 则最前面的 255 个异步处理, 接下来的 16384 个(左移 6 位 = * 64)放进队列, 之后的 3 个( 256 + 2 - 255)异步处理, 再往后的拒绝
