...
import com.github.res.UserLoginRes;
...
@NeedLogin
@ApiGroup(value = UserConst.MODULE_INFO, index = 2)
@RestController
@RequestMapping("/user")
public class BackendUserController {
    ...
    public JsonResult<UserLoginRes> login(@ApiParam(value = "用户名", must = true) String userName,
        ...
        */
        ...
        UserLoginRes user = new UserLoginRes(); // 假定这是从上面操作之后得到的用户对象(上面的 User)
        user.setId(123L).setName("abc");

        // 登录成功后将用户信息存入 session 并生成 token 后返回给前端
        String token = BackendSessionUtil.whenLogin(user);
        return JsonResult.success("登录成功", UserLoginRes.assemblyData(user, token));
    }
